...
public class ConnectionMultiplexerManager implements SessionEventListener {
    ...
    private Cache<String, ClientSessionInfoBack> sessionInfoBackCache = CacheFactory.createCache(SessionManager.C2S_BACKUP_CACHE_NAME);
    ...
    public boolean createClientSession(String connectionManagerDomain, String streamID, String hostName, String hostAddress) {
        ...
        if (address == null || LocalClientSession.isAllowed(connection)) {
            ...
            sessionInfoBackCache.put(streamID, new ClientSessionInfoBack(session));
    ...
    public void addClientSession(LocalClientSession session,String connectionManagerDomain, String streamID){
        streamIDs.put(streamID, connectionManagerDomain);

    	
        //加到路由表
        XMPPServer.getInstance().getRoutingTable().addClientRoute(session.getAddress(), session);
        ...
        sessions.put(session.getStreamID().getID(), session);
    }
    ...
    public LocalClientSession getClientSession(String connectionManagerDomain, String streamID) {
        Map<String, LocalClientSession> sessions = sessionsByManager.get(connectionManagerDomain);
        if (sessions != null&&sessions.containsKey(streamID)) {
            return sessions.get(streamID);
        }else{
        	ClientSessionInfoBack backupInfo = sessionInfoBackCache.get(streamID);
        	if(backupInfo==null){
        		Log.warn("Cache sessionInfoBackCache get null");
        		return null;
        	}
            Connection connection = new ClientSessionConnection(connectionManagerDomain, backupInfo.getHostName(), backupInfo.getHostAddress());
            LocalClientSession session =  SessionManager.getInstance().restoreClientSession(connection, streamID);
            if(session!=null){
            	addClientSession(session,connectionManagerDomain,streamID);
            }
            return session;
        }
    }
    ...
    private void removeSession(Session session) {
        ...
        sessionInfoBackCache.remove(streamID);
